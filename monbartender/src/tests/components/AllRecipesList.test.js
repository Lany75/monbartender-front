...
describe('<AllRecipesList />', () => {
  ...
  describe('it tests case with undefined listeCocktails', () => {
    jest.spyOn(React, 'useContext').mockImplementation(() => ({
      listeCocktails: undefined,
    }));

    const allRecipesList = shallow(<AllRecipesList />);

    it('should contain a LoadingMessage component', () => {
      expect(allRecipesList.find(LoadingMessage)).to.have.length(1);
      expect(allRecipesList.find('div.all-recipes-list')).to.have.length(0);
    })
  })


  describe('it tests case with defined listeCocktails', () => {
    jest.spyOn(React, 'useContext').mockImplementation(() => ({
      listeCocktails: testListCocktails,
    }));

    const allRecipesList = shallow(<AllRecipesList />);
    const divAllRecipesList = allRecipesList.find('div.all-recipes-list');

    it('should contain a div witch className="all-recipes-list"', () => {
      expect(divAllRecipesList.find(LoadingMessage)).to.have.length(0);
      expect(divAllRecipesList).to.have.length(1);
    })

    it('should contain 2 divs with className="all-recipes-title"', () => {
      expect(divAllRecipesList.find('div.all-recipes-title')).to.have.length(2);
    })

    it('should contain 2 divs with className="cocktails-list"', () => {
      expect(divAllRecipesList.find('div.cocktails-list')).to.have.length(2);
    })

    it('should contain 4 CocktailCard components', () => {
      expect(divAllRecipesList.find(CocktailsCard)).to.have.length(4);;
    })
  })
})