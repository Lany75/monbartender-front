...
describe('<DialogModifyGlass />', () => {
  ...
  const dialogModifyGlass = shallow(<DialogModifyGlass />);
  const dialog = dialogModifyGlass.find(Dialog);
  ...
  it('should contain a DialogErrorMessage component', () => {
    expect(dialogModifyGlass.find(DialogErrorMessage)).to.have.length(1);
  })
})